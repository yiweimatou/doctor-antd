    ...
    render() {
        ...
        return (
            <Spin spinning={loading}>
                <Modal visible={visible} title='新增微信' footer='' width={720} onCancel={this._onCancel}>
                ...
                <div className='image-div-topbar'>
                        <Button type='primary' onClick={() => this.setState({visible: true})}>新增微信</Button>
                        <span className='image-div-topbar-span'>在http://weixin.sogou.com中找到微信内容，复制微信的网址。</span>
                </div>
                <Row>
                {
                    list.map(item => {
                ...
                <div style={{marginTop: 20}}>
                    <Pagination total={total} pageSize={8} showTotal={total => `共${total}条`} onChange={
                        offset => this.props.list({state: 1, limit: 8, offset, category_id: WX})
                                    .then(data => this.setState({list: data.list, loading: false}))
                                    .catch(error => message.error(error))
                    }/>
                </div>
